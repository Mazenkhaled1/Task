...
@section('main')
<h2 style="text-align: center; width: 100%;">AboutUs - Header Information</h2>
...
<div class="center-table-wrapper">
    ...
    <table>
      ...
      <tbody>
        @foreach ($about_us_headers as $about_us_header)
        <tr>
          <td>{{ $about_us_header->name }}</td>
          <td>{{ $about_us_header->title }}</td>
          <td>{{ $about_us_header->description }}</td>
          <td>
            <!-- Button to open the popup with the contact ID -->
            <button class="update-btn" onclick="openModal({{ $about_us_header->id }})">تحديث</button>
          </td>        
        </tr>
        @endforeach
      </tbody>
...
<div id="updateModal" class="modal">
  <div class="modal-content">
    ...
    <form id="updateForm" action="{{ route('about_us_header.update', ':id') }}" method="POST" enctype="multipart/form-data">
      @csrf
      @method('PUT')
