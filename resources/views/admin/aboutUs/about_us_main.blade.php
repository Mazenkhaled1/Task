...
@section('main')
<h2 style="text-align: center; width: 100%;">AboutUs - Main Information</h2>
...
<div class="center-table-wrapper">
    ...
    <table>
      ...
      <tbody>
        @foreach ($about_us_mains as $about_us_main)
        <tr>
          <td>{{ $about_us_main->title }}</td>
          <td>{{ $about_us_main->subtitle }}</td>
          <td>
            <!-- عرض الصورة الخلفية -->
            <img src="{{ asset('storage/' . $about_us_main->image) }}" alt="Background Image" width="100" height="100">
          </td>
          <td>
            <!-- Button to open the popup with the contact ID -->
            <button class="update-btn" onclick="openModal({{ $about_us_main->id }})">تحديث</button>
          </td>        
        </tr>
        @endforeach
      </tbody>
...
<div id="updateModal" class="modal">
  <div class="modal-content">
    ...
    <form id="updateForm" action="{{ route('about_us_main.update', ':id') }}" method="POST" enctype="multipart/form-data">
      @csrf
      @method('PUT')
