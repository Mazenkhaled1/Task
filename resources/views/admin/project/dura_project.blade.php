...
@section('main')
<h2 style="text-align: center; width: 100%;">Project - Dura Information</h2>
...
<div class="center-table-wrapper">
    ...
    <table>
      ...
      <tbody>
        @foreach ($dura_projects as $dura_project)
        <tr>
            <td>{{ $dura_project->title }}</td>
            <td>{{ $dura_project->description }}</td>
            <td>
                <img src="{{ asset('storage/' . $dura_project->main_image) }}" alt="Background Image" width="100" height="100">        
           </td>
           <td>
               <img src="{{ asset('storage/' . $dura_project->secondary_image) }}" alt="Background Image" width="100" height="100">        
          </td>
          <td>
            <!-- Button to open the popup with the contact ID -->
            <button class="update-btn" onclick="openModal({{ $dura_project->id }})">تحديث</button>
          </td>        
        </tr>
        @endforeach
      </tbody>
...
<div id="updateModal" class="modal">
    <div class="modal-content">
      ...
      <h3 class="text-center mb-4">update project</h3>
      <form id="updateForm" action="{{ route('dura_project.update', ':id') }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="form-group">
          <label for="secondary_image">secondary_image</label>
          <input type="file" id="secondary_image" name="secondary_image" class="form-control">
        </div>
