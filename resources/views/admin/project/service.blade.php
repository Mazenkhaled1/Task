...
@section('main')
<h2 style="text-align: center; width: 100%;">Project - Service Information</h2>
...
<div class="center-table-wrapper">
  ...
  <button class="btn-add-new" onclick="openAddModal()">Add New Service</button>
    <table>
      <thead>
        <tr>
          <th>name</th>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <img src="{{ asset('storage/' . $service->image) }}" alt="Background Image" width="100" height="100">
          </td>
          <td>
            <button class="btn-update" onclick="openModal({{ $service->id }})">تحديث</button>
          </td>        
        </tr>
        @endforeach
      </tbody>
...
<div id="postModal" class="modal">
    <div class="modal-content">
      ...
      <h3 class="text-center mb-4" id="modalTitle">Add New Service</h3>
      <form id="postForm" action="{{ route('project_service.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <button type="submit" class="btn-add-new" id="submitBtn">Add service</button>
      </form>
    </div>
  </div>
...
<div id="updateModal" class="modal">
  <div class="modal-content">
    ...
    <form id="updateForm" action="{{ route('project_service.update', ':id') }}" method="POST" enctype="multipart/form-data">
      @csrf
      @method('PUT')
...
<script>
  ...
  function openAddModal() {
    ...
    document.getElementById("postForm").action = "{{ route('project_service.store') }}";
