...
<div class="center-table-wrapper">
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          <td>{{ $contact->title }}</td>
          <td>{{ $contact->breadcrumb }}</td>
          <td>
            <!-- عرض الصورة الخلفية -->
            <img src="{{ asset('storage/' . $contact->background_image) }}" alt="Background Image" width="100" height="100">
          </td>
          <td>{{ $contact->sub_title }}</td>
          <td>
            @if($contact->secondary_images)
                <!-- إذا كانت الصور الثانوية موجودة في قاعدة البيانات -->
                @php
                    $secondaryImages = json_decode($contact->secondary_images);
                @endphp
                @foreach ($secondaryImages as $image)
                    <img src="{{ asset('storage/' . $image) }}" alt="Secondary Image" width="100" height="100" style="margin-right: 5px;">
                @endforeach
            @else
          ...
          <td>
            ...
            <button class="update-btn" onclick="openModal({{ $contact->id }})">تحديث</button>
          </td>        
        </tr>
        @endforeach
      </tbody>
...
<div id="updateModal" class="modal">
  <div class="modal-content">
    ...
    <form id="updateForm" action="{{ route('contact.update', ':id') }}" method="POST" enctype="multipart/form-data">
      @csrf
      @method('PUT')
      ...
      <div class="form-group">
        ...
        <input type="text" id="breadcrumb" name="breadcrumb" class="form-control" required>
      </div>
      ...
      <div class="form-group">
        <label for="secondary_image">Secondary_images</label>
        <input type="file" id="secondary_image" name="secondary_image[]" multiple class="form-control">
      </div>
