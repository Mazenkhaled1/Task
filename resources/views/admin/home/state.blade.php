...
@section('main')
<h2 style="text-align: center; width: 100%;">Home - State Information</h2>
...
<div class="center-table-wrapper">
  ...
  <button class="btn-add-new" onclick="openAddModal()">Add New State</button>
    <table>
      <thead>
        <tr>
            <th>label</th>
      ...
      <tbody>
        @foreach ($home_states as $home_state)
        <tr>
          <td>{{ $home_state->label }}</td>
          <td>{{ $home_state->count_value }}</td>
          <td>
            <img src="{{ asset('storage/' . $home_state->image_url) }}" alt="Background Image" width="100" height="100">
          </td>
          <td>
            <button class="btn-update" onclick="openModal({{ $home_state->id }})">تحديث</button>
          </td>        
        </tr>
        @endforeach
      </tbody>
...
<div id="postModal" class="modal">
    <div class="modal-content">
      ...
      <form id="postForm" action="{{ route('home_state.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div class="form-group">
            <label for="count_value">count_value</label>
            <textarea id="count_value" name="count_value" class="form-control" required></textarea>
        </div>
...
<div id="updateModal" class="modal">
  <div class="modal-content">
    ...
    <form id="updateForm" action="{{ route('home_state.update', ':id') }}" method="POST" enctype="multipart/form-data">
      @csrf
      @method('PUT')
...
<script>
  ...
  function openAddModal() {
    ...
    document.getElementById("postForm").action = "{{ route('home_state.store') }}";
